...
// compute
void ABE2OD::SETSTATICSIZE(pairing_t pairing)
{
    ...
    //element_printf("ZR_LENGTH = %d, %B\n", ZR_SIZE, zr);
    //element_printf("G1_LENGTH = %d, %B\n", G1_SIZE, g1);
    //element_printf("G2_LENGTH = %d, %B\n", G2_SIZE, g2);
    //element_printf("GT_LENGTH = %d, %B\n", GT_SIZE, gt);

}

// element_t -> string
void ABE2OD::serl(unsigned char** str, size_t* count, element_t e)
{
...
// string -> element_t
void ABE2OD::deserl(element_t e, unsigned char* str, size_t count)
{
...
void ABE2OD::serl_array(unsigned char** arr_str, size_t* str_count, element_t *array, size_t array_length)
{
...
void ABE2OD::deserl_array(element_t* array_rec, size_t array_length, unsigned char* arr_str, size_t str_count)
{
...
// struct PK -> string
void ABE2OD::serl_PK(unsigned char** str, size_t* str_count, struct PK pk)
{
...
// string -> struct PK
void ABE2OD::deserl_PK(struct PK pk, unsigned char* str, size_t str_count)
{
...
void ABE2OD::serl_vec(unsigned char** str, size_t* str_count, std::vector<struct element_s>& vec)
{
...
void ABE2OD::deserl_vec(std::vector<struct element_s>& vec, unsigned char* str, size_t str_count)
{
...
void ABE2OD::serl_umap(unsigned char** key_str, size_t* key_str_count,
...
void ABE2OD::deserl_umap(std::map<std::string, struct element_s>& umap, unsigned char* str, size_t str_count)
{
...
// LSSS -> string
void ABE2OD::serl_lsss(char** policy_str, size_t* policy_str_count, LSSS *policy)
{
...
// string -> LSSS
void ABE2OD::deserl_lsss(LSSS *policy, char* policy_str, size_t policy_str_count)
{
...
// struct MSK -> string
void ABE2OD::serl_MSK(unsigned char** str, size_t* str_count, struct MSK msk)
{
...
// string -> struct MSK
void ABE2OD::deserl_MSK(struct MSK msk, unsigned char* str, size_t str_count)
{
...
void ABE2OD::serl_Ciphertext(unsigned char** str, size_t* str_count, struct Ciphertext cipher, size_t* policy_str_count)
{
    ...
    *str_count = (size_t) *policy_str_count+ + cipher.C1.size()+ element_length_in_bytes(cipher.C0) + element_length_in_bytes(cipher.C2)+str1_count+str2_count+str3_count;
    *str = (unsigned char*) malloc(*str_count);
    unsigned char *pointer = *str;
    ...
    memcpy(pointer,cipher.C1.c_str(), cipher.C1.size());//cipher.C1
    ...
    memcpy(pointer, vec_str1,str1_count);//Ci
...
void ABE2OD::deserl_Ciphertext(struct Ciphertext &cipher, unsigned char* str, size_t str_count, size_t policy_str_count)
{
    ...
    pointer = pointer + G1_SIZE;
    size_t n= (str_count - policy_str_count - G1_SIZE - GT_SIZE -cipher.C1.size() ) / (G1_SIZE+ G1_SIZE+ ZR_SIZE);
    //Ei
...
{
    //printf("TC serialization --- \n");
    *str_count = tc.T1.size()+ (size_t) element_length_in_bytes(tc.T0) + (size_t)element_length_in_bytes(tc.T2);
    *str = (unsigned char*) malloc(*str_count);
    unsigned char *pointer = *str;
    memcpy(pointer, tc.T1.c_str(), tc.T1.size());
...
// struct TK -> string
void ABE2OD::serl_TK(unsigned char** str, size_t* str_count, struct KeyTuple::TK tk,
...
// string -> struct TK
void ABE2OD::deserl_TK(struct KeyTuple::TK  &tk, unsigned char* str, size_t str_count, unsigned char* umap_value_str, size_t umap_value_len)//加引用才能让string attributes成功赋值
{
...
// struct DK -> string
void ABE2OD::serl_DK(unsigned char** str, size_t* str_count, struct KeyTuple::DK dk)
{
...
// string -> struct DK
void ABE2OD::deserl_DK(struct KeyTuple::DK &dk, unsigned char* str, size_t str_count)
{
